		...
		if (data.is_object())
		{
			...
			if (obj.contains("mesh")) {
				...
				if (mesh_obj.contains("material")) {
					node.mesh->material = ctx.desc_system.get_field_desc<scn::material_desc>(ctx.self, mesh_obj.at("material"));
				}
			}
...
void scn::prototype_desc::deserialize(desc::desc_system& desc_system, const json::object& data)
{
	if (data.contains("geometry")) {
		geometry = desc_system.get_field_desc<rnd::geometry_desc>(*this, data.at("geometry"));
	}
