...
void engine::game::game_init(ds::app_data_storage& data)
{
	...
	desc_sys.register_desc<scn::material_desc>("material_desc"); // TODO: move all register into scene init
	...
	assembler.register_desc_spawner(scn::mesh_node_desc::__type, [] (entt::registry& reg, entt::entity e, const desc::desc_base& base_data, const std::string_view name) {
		const auto& desc = static_cast<const scn::mesh_node_desc&>(base_data);
		scn::assemble_mesh_node(reg, e, desc, name);
	});

	assembler.register_desc_spawner(scn::object_desc::__type, [] (entt::registry& reg, entt::entity e, const desc::desc_base& base_data, const std::string_view name) {
		const auto& desc = static_cast<const scn::object_desc&>(base_data);
		scn::assemble_object(reg, e, desc, name);
	});

	assembler.register_desc_spawner(scn::animations_desc::__type, [] (entt::registry& reg, entt::entity e, const desc::desc_base& base_data, const std::string_view name) {
		const auto& desc = static_cast<const scn::animations_desc&>(base_data);
		scn::assemble_animations(reg, e, desc, name);
	});

	assembler.register_desc_spawner(scn::keyframes_desc::__type, [] (entt::registry& reg, entt::entity e, const desc::desc_base& base_data, const std::string_view name) {
		const auto& desc = static_cast<const scn::keyframes_desc&>(base_data);
		scn::assemble_keyframes(reg, e, desc, name);
	});

	assembler.register_desc_spawner(scn::bone_desc::__type, [] (entt::registry& reg, entt::entity e, const desc::desc_base& base_data, const std::string_view name) {
		const auto& desc = static_cast<const scn::bone_desc&>(base_data);
		scn::assemble_bone(reg, e, desc, name);
	});

	assembler.register_desc_spawner(scn::skinning_desc::__type, [] (entt::registry& reg, entt::entity e, const desc::desc_base& base_data, const std::string_view name) {
